...
class BackgroundUploadService {
  ...
  void _handleTaskStatusUpdate(TaskStatusUpdate update) async {
    switch (update.status) {
      case TaskStatus.complete:
        ...
        unawaited(_uploadThumbnail(update));
  ...
  Future<void> _uploadThumbnail(TaskStatusUpdate update) async {
    try {
      ...
      final entity = await _storageRepository.getAssetEntityForAsset(localAsset);
      if (entity == null) return;

      final thumbData = await entity.thumbnailDataWithSize(const ThumbnailSize(256, 256));
      if (thumbData == null) return;

      final thumbBase64 = base64Encode(thumbData);
      final serverEndpoint = Store.get(StoreKey.serverEndpoint);
      final headers = ApiService.getRequestHeaders();

      final thumbBody = {'thumbData_base64': thumbBase64};
      final thumbResponse = await http.post(
        Uri.parse('$serverEndpoint/assets/$assetId/thumbnail'),
        headers: headers,
        body: thumbBody,
      );

      if (thumbResponse.statusCode == 200) {
        _logger.info('Thumbnail uploaded for $assetId');
      } else {
        _logger.warning('Thumbnail upload failed for $assetId: ${thumbResponse.statusCode}');
      }
    } catch (error, stackTrace) {
      _logger.warning("Error uploading thumbnail: $error $stackTrace");
    }
  }
  ...
  Future<UploadTask?> getUploadTask(LocalAsset asset, {String group = kBackupGroup, int? priority}) async {
    ...
    try {
      ...
      _logger.warning("Failed to generate thumbnail for ${asset.id}: $e");
    }
    ...
    return buildUploadTask(
      ...
      metadata: metadata,
      fields: thumbBase64 != null ? {'thumbData_base64': thumbBase64} : null,
      group: group,
      priority: priority,
  ...
  Future<UploadTask?> getLivePhotoUploadTask(LocalAsset asset, String livePhotoVideoId) async {
    ...
    try {
      final thumbData = await entity.thumbnailDataWithSize(const ThumbnailSize(256, 256));
      if (thumbData != null) {
        fields['thumbData_base64'] = base64Encode(thumbData);
      }
    } catch (_) {}
