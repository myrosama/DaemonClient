...
export const deleteAssets = async (
  ...
  try {
    ...
    // --- DaemonClient Drive Delete Interceptor ---
    if (force) {
        const { daemonDrive } = await import('./daemonclient-drive');
        for (const asset of assets) {
            if ((asset as any).telegramChunks) {
                await daemonDrive.deleteMedia((asset as any).telegramChunks);
            }
        }
    }

    await deleteBulk({ assetBulkDeleteDto: { ids, force, clientDelete: true } as any });
    onAssetDelete(ids);
