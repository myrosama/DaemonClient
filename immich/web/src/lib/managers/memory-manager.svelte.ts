...
class MemoryManager {
  ...
  private async load() {
    try {
      const result = await searchMemories({ $for: asLocalTimeISO(DateTime.now()) });
      // Guard against non-array responses (e.g. when service worker is not yet
      // active on a direct navigation and Firebase Hosting returns HTML for the
      // /api/memories request instead of the Cloudflare Worker JSON).
      const memories = Array.isArray(result) ? result : [];
      this.memories = memories.filter((memory) => memory.assets.length > 0);
    } catch {
      // Swallow errors (auth failure, network error, etc.) — memories are
      // non-critical; the rest of the app should continue loading normally.
      this.memories = [];
    }
  }
}
