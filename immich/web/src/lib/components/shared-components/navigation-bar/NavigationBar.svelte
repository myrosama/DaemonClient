...
<script lang="ts">
  ...
  import { mdiBellBadge, mdiBellOutline, mdiMagnify, mdiMenu, mdiTrayArrowUp, mdiLock, mdiLockOpenVariant } from '@mdi/js';
  ...
  onMount(async () => {
    ...
    try {
      const res = await fetch(`${getBaseUrl()}/assets/zke-status`);
      if (res.ok) {
...
<nav id="dashboard-navbar" class="max-md:h-(--navbar-height-md) h-(--navbar-height) w-dvw text-sm">
  ...
  >
    <div class="flex flex-row gap-1 mx-4 items-center">
      ...
      />
      <a data-sveltekit-preload-data="hover" href={Route.photos()} class="flex items-center gap-2">
        <img src="/daemonclient-logo.png" alt="DaemonClient Logo" class="h-10 w-10 object-contain max-md:h-8 max-md:w-8" />
        {#if mediaQueryManager.isFullSidebar}
          <span class="text-lg font-bold tracking-tight text-immich-primary truncate hidden sm:block">Photos</span>
        {/if}
      </a>
    </div>
