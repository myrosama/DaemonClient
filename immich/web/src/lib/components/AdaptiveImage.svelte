<script lang="ts">
  ...
  const show = $derived.by(() => {
    ...
    return {
      ...
      spinner: !asset.thumbhash && quality.original !== 'success' && quality.preview !== 'success' && quality.thumbnail !== 'success' && !hasError,
      brokenAsset: hasError && quality.original === 'error', // Only show broken if the final fallback fails
      thumbhash: quality.thumbnail !== 'success' && quality.preview !== 'success' && quality.original !== 'success',
      thumbnail: quality.thumbnail === 'success' || (quality.thumbnail !== 'error' && quality.original !== 'success'),
      preview: quality.preview === 'success' || (quality.preview !== 'error' && quality.original !== 'success'),
      original: quality.original === 'success' && urls.original !== undefined,
    };
  });
...
<div class="relative h-full w-full overflow-hidden" bind:this={ref}>
  ...
  <div
    ...
    style:height
  >
    <!-- Removed AlphaBackground to prevent grayish squares behind images with incorrect metadata dimensions -->
