...
// DaemonClient deployment has no real-time WS backend (Firebase Hosting doesn't
// speak WebSocket and the per-user CF worker doesn't run a socket.io server).
// `reconnection: true` made the client hammer wss://photos.daemonclient.uz
// with retry attempts that timed out one-by-one, adding measurable boot
// latency for nothing. Disable reconnection and never auto-connect.
...
export const openWebsocketConnection = () => {
  // No-op for DaemonClient — there is no WS backend. Calling websocket.connect()
  // here would issue a wss:// upgrade request that Firebase Hosting refuses,
  // wasting an HTTP slot and adding to load-time network contention.
};
