...
export const SHIM_VERSION = "d2aefd385740";
...
async function handleThumbnailUpload(request, env2, uid, assetId, idToken) {
  ...
  try {
    ...
    const update = { id: assetId, telegramThumbId, thumbEncrypted: thumbEncrypted ? 1 : 0 };
    if (thumbhash)
      update.thumbhash = thumbhash;
    if (env2.DB) {
      await new D1Adapter(env2.DB).savePhoto(update);
    } else {
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "d2aefd385740";
