...
export const SHIM_VERSION = "491d524fbea4";
...
async function handleDeleteAssets(request, env2, uid, idToken) {
  ...
  if (!force) {
    if (env2.DB) {
      const adapter = new D1Adapter(env2.DB);
      for (const id of ids)
        await adapter.updatePhoto(id, { isTrashed: 1 });
    } else {
      for (const id of ids)
        await firestoreSet(env2, uid, \`photos/\${id}\`, { isTrashed: true }, idToken);
    }
    return json({});
  }
  const config2 = await getCachedConfig(env2, uid, idToken, "telegram");
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "491d524fbea4";
