...
export const SHIM_VERSION = "f676e4b8f232";
...
function isSelfHost(env2) {
  const v2 = env2.SELF_HOST;
  return v2 === "1" || v2 === "true" || v2 === true;
}
function sessionScope(env2) {
  const secret = env2.SESSION_SECRET;
  ...
  return env2.APP_IDENTIFIER || "default";
}
var init_selfhost_auth = __esm({
  "src/selfhost-auth.ts"() {
    ...
    init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
    init_performance2();
    __name(isSelfHost, "isSelfHost");
    __name(sessionScope, "sessionScope");
  }
});

// src/owner-gate.ts
async function readOwner(env2) {
  if (cachedOwner !== void 0)
    return cachedOwner;
  try {
    const row = await env2.DB.prepare("SELECT value FROM config WHERE key = ? LIMIT 1").bind(OWNER_KEY).first();
...
async function claimOwner(env2, uid) {
  try {
    await env2.DB.prepare("INSERT OR IGNORE INTO config (key, value) VALUES (?, ?)").bind(OWNER_KEY, uid).run();
    cachedOwner = void 0;
  } catch {
  }
}
async function requireOwner(env2, uid) {
  if (!env2.DB)
    return;
  const owner = await readOwner(env2);
  if (owner === void 0) {
    if (isSelfHost(env2))
...
async function requireAuth(request, env2) {
  ...
  session = await verifySignedSessionToken(token, sessionScope(env2 || {}));
  if (!session)
    throw new Error("Session expired");
  if (env2)
    await requireOwner(env2, session.uid);
  if (env2 && isSelfHost(env2))
    return session;
    ...
    D1Adapter = class {
      ...
      async getZkeConfig() {
       ...
       WHERE key IN ('zke_mode', 'zke_enabled', 'zke_password', 'zke_salt')\`
        ...
        if (!config2.zke_mode && config2.zke_enabled !== "1")
          return null;
        return {
          mode: config2.zke_mode || "server",
...
async function handleLogin(request, env2) {
  ...
  }, userSessionSecret || env2.APP_IDENTIFIER || "default");
  const userResponse = {
...
async function handleServer(request, env2, path) {
  if (path === "/api/server/config" || path === "/api/server-info/config")
    return json(serverConfig(request, env2));
  ...
  if (path === "/api/server/processor")
    return handleProcessor(request, env2);
...
function serverConfig(request, env2) {
  const externalDomain = env2.EXTERNAL_DOMAIN || (isSelfHost(env2) ? new URL(request.url).origin : "https://photos.daemonclient.uz");
  return {
    loginPageMessage: "",
    trashDays: 30,
    userDeleteDelay: 7,
...
__name(processorUrlIsSane, "processorUrlIsSane");
async function handleProcessor(request, env2) {
  const session = await requireAuth(request, env2);
  if (request.method === "GET") {
    const cfg = await getCachedConfig(env2, session.uid, session.idToken, "telegram").catch(() => null);
  ...
  if (request.method === "DELETE") {
    ...
    const db2 = new D1Adapter(env2.DB);
...
__name(handleProcessor, "handleProcessor");
...
function toAssetManifest(photo) {
  const rawChunks = typeof photo?.telegramChunks === "string" ? safeParse(photo.telegramChunks) : photo?.telegramChunks || [];
  const chunks = (Array.isArray(rawChunks) ? rawChunks : []).filter((c2) => c2 && c2.file_id).map((c2) => ({ index: Number(c2.index) || 0, file_id: String(c2.file_id) }));
...
__name(toAssetManifest, "toAssetManifest");
...
__name(earlyDedupDecision, "earlyDedupDecision");
...
__name(videoHintFromFields, "videoHintFromFields");
...
var Accept = /* @__PURE__ */ __name(class {
  #map;
  constructor(init) {
...
var AcceptEncoding = /* @__PURE__ */ __name(class {
  #map;
  constructor(init) {
    this.#map = /* @__PURE__ */ new Map();
    if (init) {
...
var AcceptLanguage = /* @__PURE__ */ __name(class {
  #map;
  constructor(init) {
    this.#map = /* @__PURE__ */ new Map();
    if (init) {
...
var CacheControl = /* @__PURE__ */ __name(class {
  maxAge;
  maxStale;
  minFresh;
  sMaxage;
...
var ContentDisposition = /* @__PURE__ */ __name(class {
  filename;
  filenameSplat;
  name;
  type;
...
function decodeFilenameSplat(value) {
  ...
  try {
    ...
    console.warn(\`Failed to decode filename from charset \${charset}:\`, error3);
    return decodedFilename;
  }
}
__name(decodeFilenameSplat, "decodeFilenameSplat");
...
var ContentType = /* @__PURE__ */ __name(class {
  boundary;
  charset;
  mediaType;
  constructor(init) {
...
var IfNoneMatch = /* @__PURE__ */ __name(class {
  tags = [];
  constructor(init) {
    if (init) {
      if (typeof init === "string") {
...
var SetCookie = /* @__PURE__ */ __name(class {
  domain;
  expires;
  httpOnly;
  maxAge;
...
__name(canonicalHeaderName, "canonicalHeaderName");
var CRLF = "\\r\\n";
...
var SuperHeaders = /* @__PURE__ */ __name(class _SuperHeaders extends Headers {
  #map;
  #setCookies = [];
  constructor(init) {
    super();
...
function createSearch(pattern) {
  ...
  if ("Buffer" in globalThis && !("Bun" in globalThis || "Deno" in globalThis)) {
    search = /* @__PURE__ */ __name((haystack, start = 0) => Buffer.prototype.indexOf.call(haystack, needle, start), "search");
  } else {
    ...
    search = /* @__PURE__ */ __name((haystack, start = 0) => {
...
var MultipartParseError = /* @__PURE__ */ __name(class extends Error {
  constructor(message) {
    super(message);
    this.name = "MultipartParseError";
  }
}, "MultipartParseError");
var MaxHeaderSizeExceededError = /* @__PURE__ */ __name(class extends MultipartParseError {
...
var MaxFileSizeExceededError = /* @__PURE__ */ __name(class extends MultipartParseError {
...
var MultipartParser = /* @__PURE__ */ __name(class {
  boundary;
  maxHeaderSize;
  maxFileSize;
  #findOpeningBoundary;
...
var MultipartPart = /* @__PURE__ */ __name(class {
  /**
...
__name(getMultipartBoundary, "getMultipartBoundary");
function isMultipartRequest(request) {
  let contentType = request.headers.get("Content-Type");
  return contentType != null && contentType.startsWith("multipart/");
}
__name(isMultipartRequest, "isMultipartRequest");
...
__name(makeFileLike, "makeFileLike");
...
__name(parseUploadRequest, "parseUploadRequest");
...
__name(makeFormDataLike, "makeFormDataLike");
...
function webAppOrigin(request, env2) {
  const configured = env2.EXTERNAL_DOMAIN;
  if (configured)
...
__name(webAppOrigin, "webAppOrigin");
...
__name(pruneFilePathCache, "pruneFilePathCache");
...
__name(EncryptionUnavailableError, "EncryptionUnavailableError");
async function getEncryptionKey(env2, uid, idToken) {
  const zkeConfig = env2.DB ? await new D1Adapter(env2.DB).getZkeConfig() : await firestoreGet(env2, uid, "config/zke", idToken);
  if (!zkeConfig)
    return null;
  const off2 = zkeConfig.enabled === false || zkeConfig.enabled === 0 || zkeConfig.enabled === "" || zkeConfig.enabled === "0" || zkeConfig.enabled === "false" || zkeConfig.enabled === void 0 || zkeConfig.enabled === null;
  if (off2)
    return null;
  if (!zkeConfig.password || !zkeConfig.salt) {
    ...
    throw new EncryptionUnavailableError(\`no \${missing}\`, isSelfHost(env2));
  }
...
async function handleAssets(request, env2, path, url) {
  ...
  try {
    ...
    if (path === "/api/assets/zke-status" && request.method === "GET") {
      const cfg = env2.DB ? await new D1Adapter(env2.DB).getZkeConfig() : await firestoreGet(env2, uid, "config/zke", idToken) || {};
    ...
    if (resourceId && path.endsWith("/dc-manifest") && request.method === "GET") {
      const photo = await loadPhotoById(env2, uid, resourceId, idToken);
      if (!photo)
        return json({ message: "Not found" }, 404);
    ...
    if (playbackMatch && request.method === "POST") {
      return handlePlaybackRendition(request, env2, uid, playbackMatch[1], idToken);
    }
      ...
      if (env2.DB) {
        ...
        if (total > 0) {
          ...
          notifications.push({
            ...
            message: \`HEIC photos and videos don't display directly in this app \\u2014 but they're safe and they work on the web. To fix them: open \${webAppOrigin(request, env2)} in any browser (your phone works too) \\u2192 Utilities \\u2192 tap "Fix HEIC", then "Fix Videos". After that they show everywhere. Advanced: connect your own media processor to convert them automatically, with no fixing step.\`,
            read: false,
...
async function handleUpload(request, env2, uid, idToken) {
  ...
  let settled = false;
  try {
    const result = await handleUploadImpl(request, env2, uid, idToken, flight);
    if (flight.key) {
...
async function handleUploadImpl(request, env2, uid, idToken, flight) {
  ...
  const parsed = await parseUploadRequest(request, async (deviceAssetId2, deviceId2, fields) => {
    ...
    try {
      const rows = await new D1Adapter(env2.DB).getPhotosByDeviceAsset(uid, deviceAssetId2, deviceId2);
        ...
        try {
          ...
          try {
            if (isEncryptedByServer && key) {
              ...
              let tmpMsgId = null;
              if (isHeic) {
                const heicUrl = config2?.heicConvertUrl;
                if (heicUrl) {
...
async function handlePlaybackRendition(request, env2, uid, assetId, idToken) {
  const photo = await loadPhotoById(env2, uid, assetId, idToken);
  if (!photo)
    return json({ message: "Asset not found" }, 404);
  if (!env2.DB)
  ...
  await new D1Adapter(env2.DB).updatePhoto(assetId, {
...
__name(handlePlaybackRendition, "handlePlaybackRendition");
...
async function purgeExpiredTombstones(env2, uid) {
  if (!env2.DB)
    return;
  try {
    await ensureDeduplicationSchema(env2.DB);
...
__name(purgeExpiredTombstones, "purgeExpiredTombstones");
var heicThumbBackfillComplete = false;
var heicThumbBackfillRunning = false;
var lastHeicThumbBackfill = 0;
async function backfillHeicThumbBatch(env2, uid, idToken) {
  if (heicThumbBackfillComplete || heicThumbBackfillRunning || !env2.DB)
    return;
  ...
  try {
    const config2 = await getCachedConfig(env2, uid, idToken, "telegram");
    subrequests++;
    const heicConvertUrl = config2?.heicConvertUrl;
    if (!heicConvertUrl) {
    ...
    for (const photo of rows) {
      ...
      try {
        ...
        if (isServerZke && serverKey === void 0) {
          serverKey = await getEncryptionKey(env2, uid, idToken);
          subrequests++;
        }
...
__name(backfillHeicThumbBatch, "backfillHeicThumbBatch");
var checksumBackfillComplete = false;
var checksumBackfillRunning = false;
async function backfillChecksumBatch(env2, uid, idToken) {
  ...
  let attempted = 0;
  let serverKey;
  const overBudget = /* @__PURE__ */ __name(() => attempted >= MAX_PHOTOS || subrequests >= SUBREQUEST_BUDGET || Date.now() - startedAt >= TIME_BUDGET_MS, "overBudget");
  try {
    await ensureDeduplicationSchema(env2.DB);
    const config2 = await getCachedConfig(env2, uid, idToken, "telegram");
    subrequests++;
...
__name(redactTelegramUrl, "redactTelegramUrl");
...
__name(evictFilePath, "evictFilePath");
...
async function facetAssetIds(env2, uid, idToken, url) {
  const albumId = url.searchParams.get("albumId");
  const personId = url.searchParams.get("personId");
  const tagId = url.searchParams.get("tagId");
  if (albumId) {
    if (env2.DB) {
      const ids = await new D1Adapter(env2.DB).getAlbumAssets(albumId);
      return new Set(ids);
    }
...
__name(facetAssetIds, "facetAssetIds");
var timelineJobCursor = 0;
async function handleTimeline(request, env2, path, url) {
  const session = await requireAuth(request, env2);
  if (env2.DB && env2.waitUntil) {
    const jobs = [
      { name: "ChecksumBackfill", run: () => backfillChecksumBatch(env2, session.uid, session.idToken) },
      { name: "HeicThumbBackfill", run: () => backfillHeicThumbBatch(env2, session.uid, session.idToken) }
    ];
...
async function getTimeBuckets(env2, uid, idToken, url) {
  ...
  const facetIds = await facetAssetIds(env2, uid, idToken, url);
...
async function handleUser(request, env2, path) {
  ...
  if (path === "/api/users" && request.method === "GET") {
    return json([await buildUserMe(env2, session)]);
  }
...
async function buildUserMe(env2, session) {
...
__name(buildUserMe, "buildUserMe");
...
__name(completeOwner, "completeOwner");
...
__name(emptySharedLink, "emptySharedLink");
...
__name(safeVisibility, "safeVisibility");
...
async function handleSyncStream(request, env2) {
  ...
  ).bind(session.uid).all()).results.map((r2) => ({ ...r2, _id: r2.id, originalFileName: r2.fileName })) : await firestoreQuery(env2, session.uid, "photos", session.idToken, "fileCreatedAt", "DESCENDING");
  const adapter = env2.DB ? new D1Adapter(env2.DB) : null;
  const resetNeeded = adapter ? await adapter.getJsonConfig("syncResetEpoch") !== SYNC_RESET_EPOCH : false;
  const deletedPhotos = env2.DB ? (await env2.DB.prepare(
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      if (resetNeeded && adapter) {
        ...
        send2({ type: "SyncResetV1", data: {}, ack: "SyncResetV1|reset" });
        send2({ type: "SyncCompleteV1", data: {}, ack: \`SyncCompleteV1|\${(/* @__PURE__ */ new Date()).toISOString()}\` });
        controller.close();
        return;
      }
      send2({
      ...
      for (const id of loserIds) {
        send2({ type: "AssetDeleteV1", data: { assetId: id }, ack: \`AssetDeleteV1|\${id}\`, ids: [id] });
      }
  ...
  if (env2.DB && env2.waitUntil) {
    const jobs = [
      { name: "ChecksumBackfill", run: () => backfillChecksumBatch(env2, session.uid, session.idToken) },
      { name: "LivePhotoRepair", run: () => repairLivePhotoLinks(env2, session.uid) },
      { name: "ExifBackfill", run: () => backfillExifBatch(env2, session.uid, session.idToken) },
      { name: "HeicThumbBackfill", run: () => backfillHeicThumbBatch(env2, session.uid, session.idToken) },
      { name: "Tombstones", run: () => purgeExpiredTombstones(env2, session.uid) }
    ];
...
__name(deriveDriveKey, "deriveDriveKey");
...
__name(encryptChunkAes, "encryptChunkAes");
...
__name(decryptChunkAes, "decryptChunkAes");
...
__name(responseXml, "responseXml");
...
__name(buildPropfindXml, "buildPropfindXml");
...
async function authenticateDav(request, env2) {
  if (!env2.DB)
    return null;
  const creds = parseBasicAuth(request.headers.get("Authorization"));
  if (!creds)
    return null;
  const cfg = await new D1Adapter(env2.DB).getJsonConfig("dav");
...
__name(authenticateDav, "authenticateDav");
...
__name(getDriveKey, "getDriveKey");
...
__name(davText, "davText");
async function handleWebDav(request, env2, url) {
  ...
  let auth;
  try {
    auth = await authenticateDav(request, env2);
  } catch {
...
__name(handleWebDav, "handleWebDav");
...
function isNewerVersion(latest, current) {
  const parse = /* @__PURE__ */ __name((v2) => {
...
__name(isNewerVersion, "isNewerVersion");
async function readCache(env2) {
  if (!env2.DB)
    return null;
  try {
    const row = await env2.DB.prepare("SELECT value FROM config WHERE key = ? LIMIT 1").bind(CACHE_KEY).first();
    if (!row?.value)
      return null;
...
async function writeCache(env2, value) {
  if (!env2.DB)
    return;
  try {
    await env2.DB.prepare("INSERT OR REPLACE INTO config (key, value) VALUES (?, ?)").bind(CACHE_KEY, JSON.stringify({ value, at: Date.now() })).run();
  } catch {
  }
}
__name(writeCache, "writeCache");
async function getUpdateStatus(env2, force = false) {
  const currentVersion = env2.BUILD_VERSION || "0.0.0";
  if (!force) {
    const cached = await readCache(env2);
    if (cached && Date.now() - cached.at < CACHE_TTL_MS) {
      return { ...cached.value, currentVersion };
    }
  }
  const repo = env2.UPDATE_REPO || DEFAULT_REPO;
  const base = {
    currentVersion,
    latestVersion: null,
  ...
  try {
    ...
    await writeCache(env2, status);
    return status;
  } catch (e2) {
    return { ...base, error: e2?.message || "update check failed" };
  }
}
__name(getUpdateStatus, "getUpdateStatus");

// src/index.ts
init_cached_config();
async function handleSelfHostStatus(request, env2) {
  let session;
  try {
    session = await requireAuth(request, env2);
  } catch {
    return json({ message: "Not authenticated" }, 401);
  }
  const telegram = await getCachedConfig(env2, session.uid, session.idToken, "telegram").catch(() => null);
  ...
  if (env2.DB) {
    try {
      const row = await env2.DB.prepare("SELECT COUNT(*) AS n FROM photos WHERE ownerId = ? AND (isTrashed = 0 OR isTrashed IS NULL)").bind(session.uid).first();
      photoCount = row?.n ?? null;
    } catch {
      photoCount = null;
    }
  }
  const update = await getUpdateStatus(env2).catch(() => null);
  return json({
    mode: isSelfHost(env2) ? "self-hosted" : "managed",
    worker: { version: WORKER_VERSION, build: env2.BUILD_VERSION || null, database: env2.DB ? "connected" : "missing" },
...
__name(handleSelfHostStatus, "handleSelfHostStatus");
var WORKER_VERSION = "f676e4b8f232";
  ...
  async fetch(request, env2, ctx) {
    ...
    }
    {
      ...
      if (davUrl.pathname === "/dav" || davUrl.pathname.startsWith("/dav/")) {
        return handleWebDav(request, env2, davUrl);
      }
    }
    ...
    try {
      ...
      if (path === "/api/health" && request.method === "GET") {
        ...
        response = await handleSelfHostStatus(request, env2);
