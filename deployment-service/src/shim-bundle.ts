...
export const SHIM_VERSION = "4bc3c89a5944";
...
async function fetchTelegramThumb(botToken, channelId, rawData, fileName, mimeType, isImage, isHeic) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (res.status === 429 || j?.error_code === 429) {
        await sleep(Math.min((j?.parameters?.retry_after || 1) * 1e3, 5e3));
        continue;
      }
      if (!j?.ok) {
        if (usePhoto) {
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "4bc3c89a5944";
