...
export const SHIM_VERSION = "910797a174b1";
...
init_virtual_unenv_global_polyfill_cloudflare_unenv_preset_node_console();
init_performance2();
function earlyDedupDecision(rows) {
  if (rows.length !== 1)
    return { short: false };
  const row = rows[0];
  if (!row.checksum)
...
async function handleUploadImpl(request, env2, uid, idToken) {
  ...
  const parsed = await parseUploadRequest(request, async (deviceAssetId2, deviceId2) => {
    if (!env2.DB)
      return null;
    try {
...
async function handleWebDav(request, env2, url) {
  ...
  const auth = await authenticateDav(request, env2);
  if (!auth) {
  ...
  if (method === "GET" || method === "HEAD") {
    ...
    if (method === "HEAD") {
      return new Response(null, { status: 200, headers: { "Content-Type": ctype, "Content-Length": String(node.fileSize || 0), "Accept-Ranges": "bytes" } });
    }
    ...
    const total = parts.reduce((n2, p2) => n2 + p2.byteLength, 0);
    ...
    if (range) {
      ...
      return new Response(slice, { status: 206, headers: { "Content-Type": ctype, "Content-Range": \`bytes \${range.start}-\${range.end}/\${full.byteLength}\`, "Content-Length": String(slice.byteLength), "Accept-Ranges": "bytes" } });
    }
    return new Response(full, { status: 200, headers: { "Content-Type": ctype, "Content-Length": String(full.byteLength), "Accept-Ranges": "bytes" } });
  }
  if (method === "PUT") {
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "910797a174b1";
