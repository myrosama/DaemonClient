...
export const SHIM_VERSION = "975628f3d738";
...
async function requireAuth(request, env2) {
  ...
  try {
    ...
    if (payload.exp * 1e3 < Date.now()) idTokenExpired = true;
  } catch {
    idTokenExpired = true;
  }
...
function fromFirestoreValue(val) {
  if (val.stringValue !== void 0) return val.stringValue;
  if (val.integerValue !== void 0) return Number(val.integerValue);
  if (val.doubleValue !== void 0) return val.doubleValue;
  if (val.booleanValue !== void 0) return val.booleanValue;
  if (val.nullValue !== void 0) return null;
  if (val.timestampValue !== void 0) return val.timestampValue;
    ...
    D1Adapter = class _D1Adapter {
      ...
      async getZkeConfig() {
       ...
       WHERE key IN ('zke_mode', 'zke_enabled', 'zke_password', 'zke_salt')\`
        ...
        if (!config2.zke_mode) return null;
        return {
      ...
      async listFiles(ownerId, parentId) {
        ...
        const result = await this.db.prepare(query).bind(...bindings).all();
        return (result.results || []).map(_D1Adapter.normalizeFile);
      }
      async getFile(id) {
...
async function handleServer(request, env2, path) {
  ...
  if (path === "/api/server/telegram-config") return handleTelegramConfig(request, env2);
  if (path === "/api/server/zke-config") return handleZkeConfig(request, env2);
  return json({ message: "Not found" }, 404);
}
...
function extractJpegGps(buf) {
  try {
    if (buf[0] !== 255 || buf[1] !== 216) return null;
    let i = 2;
...
async function handleUpdateAsset(request, env2, uid, assetId, idToken) {
  ...
  if (dto.isFavorite !== void 0) updates.isFavorite = dto.isFavorite ? 1 : 0;
  if (dto.isArchived !== void 0) updates.visibility = dto.isArchived ? "archive" : "timeline";
  if (dto.isTrashed !== void 0) updates.isTrashed = dto.isTrashed ? 1 : 0;
  if (dto.description !== void 0) updates.description = dto.description;
  if (dto.latitude !== void 0) updates.latitude = dto.latitude;
  if (dto.longitude !== void 0) updates.longitude = dto.longitude;
...
async function handleDeleteAssets(request, env2, uid, idToken) {
  ...
  if (!force) {
    if (env2.DB) {
      ...
      for (const id of ids) await firestoreSet(env2, uid, \`photos/\${id}\`, { isTrashed: true }, idToken);
    }
    return json({});
  }
  const config2 = await getCachedConfig(env2, uid, idToken, "telegram");
...
async function handleBulkUpdate(request, env2, uid, idToken) {
  ...
  const ids = body.ids || [];
  const updates = {};
  if (body.isFavorite !== void 0) updates.isFavorite = body.isFavorite;
  if (body.visibility !== void 0) updates.visibility = body.visibility;
  if (body.isTrashed !== void 0) updates.isTrashed = body.isTrashed;
  if (Object.keys(updates).length > 0) {
    if (env2.DB) {
      const adapter = new D1Adapter(env2.DB);
      for (const id of ids) {
...
async function fetchTelegramThumb(botToken, channelId, rawData, fileName, mimeType) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    if (attempt > 0) await sleep(Math.min(500 * Math.pow(2, attempt), 6e3));
    try {
...
async function handleReplaceVideo(request, env2, uid, assetId, idToken) {
  const photo = await loadPhotoById(env2, uid, assetId, idToken);
  if (!photo) return json({ message: "Asset not found" }, 404);
  if (!env2.DB) return json({ message: "Not supported without D1" }, 400);
  const config2 = await getCachedConfig(env2, uid, idToken, "telegram");
  if (!config2) return json({ message: "No config" }, 500);
  const botToken = config2.botToken || config2.bot_token;
  const channelId = config2.channelId || config2.channel_id;
...
async function handleOriginal(request, env2, uid, assetId, idToken) {
  ...
  if (photo.telegramChunks && photo.telegramChunks?.length > 0) {
    ...
    const getChunk = /* @__PURE__ */ __name(async (chunk) => {
      ...
      if (isServerZke && key) data2 = await decryptChunk(data2, key);
...
async function tgFetchWithRetry(url, options, maxRetries = 3) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    if (botToken && isSendUrl(url)) await paceSend(botToken);
  ...
  try {
    ...
    if (albumMatch) {
      ...
      if (request.method === "DELETE") {
        ...
        else await firestoreDelete(env2, uid, \`albums/\${albumId}\`, idToken);
...
async function handlePolicy(request, env2, path) {
  ...
  if (path === "/api/policy/upload-session" && request.method === "POST") {
    ...
    if (!takeToken(uid, "upload")) return json({ message: "Rate limited", retryAfterMs: 2e3 }, 429);
...
async function handleDrive(request, env2, path, url) {
  if (!env2.DB) return json({ message: "Drive requires a per-user database" }, 503);
  const session = await requireAuth(request, env2);
  const uid = session.uid;
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "975628f3d738";
