...
export const SHIM_VERSION = "8bd82c7031d4";
...
async function verifyLocalCredentials(db, email, password) {
  const normalized = String(email || "").trim().toLowerCase();
  let row = null;
  try {
    row = await db.prepare("SELECT id, email, passwordHash, name, isAdmin FROM users WHERE email = ? LIMIT 1").bind(normalized).first();
  } catch {
    row = null;
  }
  if (!row || !row.passwordHash) {
    await verifyPassword(password || "x", DUMMY_RECORD);
    return null;
  }
...
async function getLocalUserById(db, id) {
  try {
    const row = await db.prepare("SELECT id, email, name, isAdmin FROM users WHERE id = ? LIMIT 1").bind(id).first();
    if (!row)
      return null;
    return {
      id: row.id,
...
async function changeLocalPassword(db, userId, currentPassword, newPassword) {
  if (typeof newPassword !== "string" || newPassword.length < 10) {
    return { ok: false, error: "New password must be at least 10 characters." };
  }
  const row = await db.prepare("SELECT passwordHash FROM users WHERE id = ? LIMIT 1").bind(userId).first();
  if (!row?.passwordHash)
  ...
  await db.prepare("UPDATE users SET passwordHash = ?, updatedAt = ? WHERE id = ?").bind(next, (/* @__PURE__ */ new Date()).toISOString(), userId).run();
  return { ok: true };
}
var PASSWORD_ITERATIONS, SALT_BYTES, KEY_BITS, ALGORITHM, DUMMY_RECORD;
var init_selfhost_auth = __esm({
  "src/selfhost-auth.ts"() {
    ...
    __name(parsePasswordRecord, "parsePasswordRecord");
    __name(timingSafeEqual, "timingSafeEqual");
    __name(verifyPassword, "verifyPassword");
    __name(isSelfHost, "isSelfHost");
    __name(sessionScope, "sessionScope");
    DUMMY_RECORD = \`\${ALGORITHM}$\${PASSWORD_ITERATIONS}$AAAAAAAAAAAAAAAAAAAAAA==$AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=\`;
    __name(verifyLocalCredentials, "verifyLocalCredentials");
    __name(getLocalUserById, "getLocalUserById");
    __name(changeLocalPassword, "changeLocalPassword");
  }
});
...
async function handleAuth(request, env2, path) {
  ...
  if (path === "/api/auth/change-password" && request.method === "POST") {
    return handleChangePassword(request, env2);
  }
  return json({ message: "Not found" }, 404);
}
__name(handleAuth, "handleAuth");
async function handleChangePassword(request, env2) {
  if (!isSelfHost(env2))
    return json({ message: "Not available on managed accounts" }, 404);
  if (!env2.DB)
    return json({ message: "No database bound" }, 500);
  let session;
  try {
    const { requireAuth: requireAuth2 } = await Promise.resolve().then(() => (init_helpers(), helpers_exports));
    session = await requireAuth2(request, env2);
  } catch {
  ...
  const result = await changeLocalPassword(
    env2.DB,
    session.uid,
    body.currentPassword || body.password || "",
    body.newPassword || ""
  );
  if (!result.ok)
    return json({ message: result.error }, 400);
  const user = await getLocalUserById(env2.DB, session.uid);
...
async function handleLogin(request, env2) {
  ...
  if (isSelfHost(env2)) {
    return handleSelfHostLogin(env2, email, password);
  }
...
__name(sessionCookieHeaders, "sessionCookieHeaders");
async function handleSelfHostLogin(env2, email, password) {
  if (!env2.DB) {
  ...
  const user = await verifyLocalCredentials(env2.DB, email, password);
  if (!user) {
    return json({ message: "Incorrect email or password" }, 401);
  }
  let scope;
  try {
    scope = sessionScope(env2);
  } catch (e2) {
    console.error("[auth] refusing to issue a session:", e2?.message);
...
__name(handleSelfHostLogin, "handleSelfHostLogin");
  ...
  if (env2.DB && env2.waitUntil) {
    env2.waitUntil(
      backfillChecksumBatch(env2, session.uid, session.idToken).catch(
        (err) => console.log("[ChecksumBackfill] timeline dispatch failed:", err?.message)
      )
    );
    env2.waitUntil(
      backfillHeicThumbBatch(env2, session.uid, session.idToken).catch(
        (err) => console.log("[HeicThumbBackfill] timeline dispatch failed:", err?.message)
      )
    );
  }
...
async function authenticateDav(request, env2) {
  ...
  return timingSafeEqual2(supplied, cfg.tokenHash) ? { uid: cfg.uid } : null;
}
...
var WORKER_VERSION = "8bd82c7031d4";
    ...
    try {
      ...
      if (path === "/api/health" && request.method === "GET") {
        ...
        try {
          ...
          allowedTarget = t2.protocol === "https:" && (t2.hostname === "api.telegram.org" || t2.hostname.endsWith(".telegram.org"));
        } catch {
          allowedTarget = false;
        }
        if (!target) {
