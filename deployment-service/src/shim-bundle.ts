...
export const SHIM_VERSION = "2177f7698abb";
...
async function handleSyncStream(request, env2) {
  ...
  ).bind(session.uid).all()).results.map(D1Adapter.normalizeRow) : await firestoreQuery(env2, session.uid, "photos", session.idToken, "fileCreatedAt", "DESCENDING");
  const adapter = env2.DB ? new D1Adapter(env2.DB) : null;
  const resetNeeded = adapter ? await adapter.getJsonConfig("syncResetEpoch") !== SYNC_RESET_EPOCH : false;
  const deletedPhotos = adapter ? (await adapter.queryPhotos({ ownerId: session.uid, isTrashed: 1 })).map(D1Adapter.normalizeRow) : [];
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for (const photo of photos) {
        ...
        if (seenChecksums.has(csum)) {
          send2({ type: "AssetDeleteV1", data: { assetId: photo._id }, ack: \`AssetDeleteV1|\${photo._id}\`, ids: [photo._id] });
          continue;
        }
  ...
  if (env2.DB && env2.waitUntil) {
    env2.waitUntil(
      backfillHeicThumbBatch(env2, session.uid, session.idToken).catch(
        (err) => console.log("[HeicThumbBackfill] dispatch failed:", err?.message)
      )
    );
  }
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "2177f7698abb";
