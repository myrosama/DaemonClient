...
export const SHIM_VERSION = "b5018a98dce7";
...
async function handleDeleteAssets(request, env2, uid, idToken) {
  ...
  for (const id of ids) {
    ...
    if (env2.DB) {
      await new D1Adapter(env2.DB).deletePhoto(id);
    } else {
...
async function handleSyncStream(request, env2) {
  ...
  const photos = env2.DB ? (await new D1Adapter(env2.DB).queryPhotos({ ownerId: session.uid, isTrashed: 0, orderBy: "fileCreatedAt DESC" })).map(D1Adapter.normalizeRow) : await firestoreQuery(env2, session.uid, "photos", session.idToken, "fileCreatedAt", "DESCENDING");
  const stream = new ReadableStream({
    async start(controller) {
...
// src/index.ts
init_helpers();
var WORKER_VERSION = "b5018a98dce7";
