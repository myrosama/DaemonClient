        ...
        <div className="min-h-screen bg-[#05080F] text-white font-sans selection:bg-indigo-500 selection:text-white overflow-x-hidden scroll-smooth">
            ...
            <motion.section id="download" className="py-24 relative bg-[#05080F] border-t border-gray-800/50" initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-100px" }} transition={{ duration: 0.8 }}>
                <div className="container mx-auto px-6">
                    ...
                    <AnimatePresence>
                        {activeDownload === 'cli' && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden"><div className="max-w-3xl mx-auto bg-[#0B0F19] rounded-2xl overflow-hidden border border-gray-800 shadow-2xl"><div className="bg-[#151926] px-6 py-3 border-b border-gray-800 flex items-center justify-between"><div className="flex gap-2"><div className="w-3 h-3 rounded-full bg-red-500/80"></div><div className="w-3 h-3 rounded-full bg-yellow-500/80"></div><div className="w-3 h-3 rounded-full bg-green-500/80"></div></div><span className="text-xs text-gray-500 font-mono font-bold">TERMINAL</span></div><div className="p-8 font-mono text-sm"><div><p className="text-gray-500 mb-3 uppercase text-xs font-bold tracking-wider">Install</p><div className="flex items-center justify-between bg-indigo-950/20 border border-indigo-500/20 p-4 rounded-xl group transition-colors hover:border-indigo-500/40"><div className="flex gap-3 text-gray-300"><span className="text-indigo-400 select-none">$</span><code>pip install daemonclient</code></div><button className="text-gray-500 hover:text-white transition-colors opacity-0 group-hover:opacity-100" onClick={() => navigator.clipboard.writeText('pip install daemonclient')} title="Copy"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></button></div></div>{/* A "Standalone Binary" row for Linux/Windows/macOS lived here behind
                            href="#". No binary has ever been built or released, so all three
                            were dead links. Removed rather than restyled — a download that
                            goes nowhere is worse than no download offered. */}</div></div></motion.div>)}
                        {activeDownload === 'desktop' && (<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="text-center text-gray-400 py-8"><p>Desktop Sync is not built yet. Follow <a href="https://t.me/daemonclient" className="text-indigo-400 underline">Telegram</a> for updates.</p></motion.div>)}
