...
const env: any = { APP_IDENTIFIER: 'test-app', SESSION_SECRET: TEST_SCOPE, FIREBASE_API_KEY: '' };
...
describe('user directory stubs', () => {
  it('GET /api/users returns an array containing the current user', async () => {
    const res = await handleUser(await req('/api/users'), env, '/api/users');
  ...
  it('GET /api/users/{self} returns the current user', async () => {
    const res = await handleUser(await req('/api/users/u1'), env, '/api/users/u1');
  ...
  it('GET /api/users/{other} returns a shaped 404, never a bare {}', async () => {
    const res = await handleUser(await req('/api/users/someone-else'), env, '/api/users/someone-else');
  ...
  it('preferences disable tags (cannot work in the isolated model)', async () => {
    const res = await handleUser(await req('/api/users/me/preferences'), env, '/api/users/me/preferences');
    const body = (await res.json()) as any;
